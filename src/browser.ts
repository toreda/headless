...
export class ArmorBrowser {
	...
	public async load(url: string, method: string, options: ArmorBrowserRequestOptions): Promise<ArmorBrowserResponse> {
		options.method.update(method);
		const request = new ArmorBrowserRequest(this.events, url, options);
		return await request.execute();
	}

	public async get(url: string, options: ArmorBrowserRequestOptions): Promise<ArmorBrowserResponse> {
		return await this.load(url, 'get', options);
	}

	public async post(url: string, options: ArmorBrowserRequestOptions): Promise<ArmorBrowserResponse> {
